...
**更新** 新增 Docker 容器化部署章节，涵盖多阶段构建、镜像优化、容器编排、健康检查、日志管理、备份策略等完整运维指导。
...
- **Docker 支持**：多阶段构建、容器编排、健康检查、日志管理
...
J["Docker 容器化"] --> K["多阶段构建<br/>deps → builder → runner"]
K --> L["Alpine Linux 基础镜像"]
K --> M["非 root 用户运行"]
K --> N["健康检查与日志"]
```
...
- [DOCKER_DEPLOY.md](file://DOCKER_DEPLOY.md#L1-L233)
- [Dockerfile](file://Dockerfile#L1-L60)
- [docker-compose.yml](file://docker-compose.yml#L1-L34)
...
#### 多阶段构建架构
- **deps 阶段**：安装 Node.js 20-alpine 和 pnpm，构建依赖
- **builder 阶段**：复制依赖，构建 Next.js 应用，生成 standalone 输出
- **runner 阶段**：精简运行时镜像，只包含必要文件

```mermaid
flowchart LR
A["Dockerfile"] --> B["deps 阶段<br/>安装依赖"]
B --> C["builder 阶段<br/>构建应用"]
C --> D["runner 阶段<br/>精简运行时"]
D --> E["最终镜像<br/>Alpine Linux + 非 root 用户"]
```
...
- **数据持久化**：
  ...
  - font-mini：生成的精简字体
  - font-temp：临时文件
- **健康检查**：每30秒检查一次 HTTP 200 响应
- **日志管理**：json-file 驱动，单文件最大10MB，最多3个文件

**章节来源**
- [Dockerfile](file://Dockerfile#L11-L13)
- [Dockerfile](file://Dockerfile#L32-L60)
- [docker-compose.yml](file://docker-compose.yml#L10-L27)

#### 部署脚本与自动化
- **一键部署**：支持 Windows PowerShell、CMD、Linux/macOS
- **手动部署**：docker-compose build/build --no-cache/up -d/logs/down
- **常用命令**：状态查看、重启、进入容器、资源监控、镜像更新、完全清理
- **构建优化**：使用 .dockerbuildconfig 配置 BuildKit、并行构建等

**章节来源**
- [DOCKER_DEPLOY.md](file://DOCKER_DEPLOY.md#L9-L47)
...
- 字体处理
  ...
  - WOFF2 输出采用"TTFTTF -> ttf2woff2"两步法，确保兼容性与质量。
  - 文本去重减少重复字形提取次数。
- I/O 与缓存
...
- [Dockerfile](file://Dockerfile#L115-L119)
...
- **Docker 故障排除**
  ...
  - 权限问题：确保字体目录有正确权限（chmod -R 777）
  - 容器无法启动：查看详细日志 docker-compose logs font-subsetting-tool
...
- [DOCKER_DEPLOY.md](file://DOCKER_DEPLOY.md#L120-L155)

## 结论
本项目以 Next.js 为基础，结合本地文件系统实现"会话隔离 + 永久备份"的数据模型，配合 fontmin/ttf2woff2/archiver 完成高效的字体子集化与分发。**新增的 Docker 容器化支持**进一步提升了部署效率、运行时稳定性与运维便利性。生产部署建议围绕"隔离、备份、可观测、限流、缓存控制、容器化"展开，辅以自动化与容量规划，确保稳定与可扩展。
...
- **一键部署**：支持 Windows PowerShell、CMD、Linux/macOS 三套脚本
- **手动部署**：docker-compose build/build --no-cache/up -d/logs/down

#### 配置说明
- **端口配置**：默认端口 6739，可通过 docker-compose.yml 修改
- **数据持久化**：font-source、font-mini、font-temp 三个目录自动挂载
- **环境变量**：NODE_ENV、PORT、HOSTNAME 等可通过环境变量配置

#### 常用命令
- 查看运行状态：docker-compose ps
- 重启服务：docker-compose restart
- 进入容器：docker-compose exec font-subsetting-tool sh
- 查看资源占用：docker stats font-subsetting-tool
- 更新镜像：docker-compose pull && docker-compose up -d
- 完全清理：docker-compose down --rmi all -v

**章节来源**
- [DOCKER_DEPLOY.md](file://DOCKER_DEPLOY.md#L3-L107)
- [docker-compose.yml](file://docker-compose.yml#L51-L58)

#### 架构说明
- **多阶段构建**：deps → builder → runner 三个阶段
- **镜像优化**：Alpine Linux、非 root 用户、健康检查、日志管理
- **安全配置**：非 root 用户运行，限制内存使用，健康检查

**章节来源**
- [DOCKER_DEPLOY.md](file://DOCKER_DEPLOY.md#L108-L155)
- [Dockerfile](file://Dockerfile#L110-L119)

#### 故障排除
- **端口被占用**：修改 docker-compose.yml 中的端口映射
- **权限问题**：chmod -R 777 font-source font-mini font-temp
- **容器无法启动**：docker-compose logs font-subsetting-tool && docker-compose build --no-cache
- **内存不足**：在 docker-compose.yml 中限制内存使用
...
#### 生产环境建议
- **反向代理**：建议使用 Nginx 作为反向代理，启用 HTTPS
- **日志配置**：json-file 驱动，单文件最大10MB，最多3个文件
- **健康检查**：每30秒检查一次 HTTP 200 响应
- **监控**：使用 docker stats 监控资源占用
...
- 部署脚本
  ...
  - **新增 Docker 部署脚本**：支持一键构建、启动、停止、重启、查看日志、清理容器和镜像。
- 最佳实践
  - 蓝绿/滚动发布，灰度流量。
  - 部署前健康检查，失败回滚。
  - 配置与密钥通过环境变量注入，不硬编码。
